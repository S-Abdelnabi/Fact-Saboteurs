...
from dataloader_retrieval_checker import DataLoaderTest
...
def save_to_file(all_predict, outpath):
    with open(outpath, "w") as out:
        ...
        for key, values in all_predict.items():
            new_evidence = []
            for i in range(args.evi_num):
                paraphrased_group = values[i]
                sorted_values = sorted(paraphrased_group, key=lambda x:x[-1], reverse=False)
                if count == 0: print(sorted_values)
                count += 1
                new_evidence.append(sorted_values[:1])               
            data = json.dumps({"id": key, "paraphrased_evidence": new_evidence})
            out.write(data + "\n")
...
def eval_model(model, validset_reader):
    ...
    all_predict = dict()
    #count = 0 
    for inp_tensor, msk_tensor, seg_tensor, ids, evi_list, evi_groups in validset_reader:
        #count += 1
        ...
        for i in range(len(probs)):
            if ids[i] not in all_predict:
                all_predict[ids[i]] = [[] for i in range(args.evi_num)]
            all_predict[ids[i]][evi_groups[i]].append(evi_list[i] + [probs[i]])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--evi_num", type=int, default=2, help='Evidence num.')
