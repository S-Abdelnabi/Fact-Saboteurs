...
class DataLoaderTest(object):
    ...
    def __init__(self, data_path, tokenizer, args, cuda=True, batch_size=64):
        ...
        inputs, ids, evi_list, evi_groups = self.read_file(data_path)
        self.inputs = inputs
        self.ids = ids
        self.evi_list = evi_list
        self.evi_groups = evi_groups 
    ...
    def read_file(self, data_path):
        ...
        with open(data_path) as fin:
            for step, line in enumerate(fin):
                ...
                id = instance['id']
                all_paraphrased_instances = instance['evidence_paraphrased']
                for evi_idx,evidence in enumerate(instance['orig_evidence']):
                    old_paraphrases = []
                    paraphrased_group = all_paraphrased_instances[evi_idx]
                    for paraphrased_evi in paraphrased_group: 
                        if paraphrased_evi in old_paraphrases: continue                  
                        ids.append(id)
                        evidence_group.append(evi_idx)
                        inputs.append([self.process_sent(claim), self.process_sent(paraphrased_evi)])
                        evi_list.append([evidence[0],evidence[1],paraphrased_evi,1])
                        old_paraphrases.append(paraphrased_evi)
        #print(len(inputs))
        #print(len(ids)) 
        #print(len(evi_list)) 
        #print(len(evidence_group))   
        #print('***') 
        #print(inputs[20])
        #print(ids[20]) 
        #print(evi_list[20]) 
        #print(evidence_group[20])         
        return inputs, ids, evi_list, evidence_group
    ...
    def next(self):
        ...
        if self.step < self.total_step:
            ...
            evi_groups = self.evi_groups[self.step * self.batch_size: (self.step + 1) * self.batch_size]                                  
            ...
            self.step += 1
            return inp_tensor_input, msk_tensor_input, seg_tensor_input, ids, evi_list,evi_groups
        else:
            self.step = 0
            raise StopIteration()