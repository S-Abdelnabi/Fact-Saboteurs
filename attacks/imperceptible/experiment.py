...
from roberta_model import stance_model
from bert_stance_model import inference_model
...
class MnliObjective():
  ...
  def objective(self) -> Callable[[List[float]], float]:
    def _objective(perturbations: List[float]) -> float:
      ...
      predict = target_model_predict(self.model, input_ids, input_mask, segment_ids, self.arch_type) ##do we need softmax here??
  ...
  def differential_evolution(self, verbose=False, maxiter=3, popsize=32, polish=False) -> str:
    ...
    attack_success = not (selection == inp_selection)
    return  {
...
class MnliTargetedObjective(MnliObjective):
  ...
  def __init__(self, model, arch_type, tokenizer, input: str, claim: str, label: int, target: int, max_perturbs: int):
    super().__init__(model, arch_type, tokenizer, input, claim, label, max_perturbs)
    self.target = target
...
class InvisibleCharacterTargetedMnliObjective(MnliTargetedObjective, InvisibleCharacterObjective):
  ...
  def __init__(self, model, arch_type, tokenizer, input: str, claim: str, label:int, target: int, max_perturbs: int, invisible_chrs: List[str] = [ZWJ,ZWSP,ZWNJ]):
    super().__init__(model, arch_type, tokenizer, input, claim, label, target, max_perturbs)
    self.invisible_chrs = invisible_chrs


class HomoglyphTargetedMnliObjective(MnliTargetedObjective, HomoglyphObjective):
  
  def __init__(self, model, arch_type, tokenizer, input: str, claim: str, label:int, target: int, max_perturbs: int, homoglyphs: Dict[str,List[str]] = intentionals):
    super().__init__(model, arch_type, tokenizer, input, claim, label, target, max_perturbs)
    self.homoglyphs = homoglyphs
    self.glyph_map = []
    for i, char in enumerate(self.input):
...
class DeletionTargetedMnliObjective(MnliTargetedObjective, DeletionObjective):
  ...
  def __init__(self, model, arch_type, tokenizer, input: str, claim: str, label:int, target: int, max_perturbs: int, del_chr: str = BKSP, ins_chr_min: str = '!', ins_chr_max: str = '~'):
    super().__init__(model, arch_type, tokenizer, input, claim, label, target, max_perturbs)
...
def mnli_experiment(model, objective, data, file, min_budget, max_budget, maxiter, popsize, exp_label, overwrite,tokenizer,arch_type, max_seq_length):
  ...
  for test in data:
    if test['id_in_file'] not in perturbs[exp_label]['0']:
      ...
      predict_logits = target_model_predict(model, input_ids, input_mask, segment_ids, arch_type)
      probs = softmax(predict_logits, dim=1).cpu().detach().numpy()[0] ##need to check this size.
  ...
  with tqdm(total=len(data)*(max_budget-min_budget+1), desc="Adv. Examples") as pbar:
    for budget in range(min_budget, max_budget+1):
      ...
      for test in data:
        if perturbs[exp_label]['0'][test['id_in_file']]['input_prediction_correct'] == False or perturbs[exp_label]['0'][test['id_in_file']]['correct_label_index'] == label_map['NOT ENOUGH INFO']: 
          sleep(0.1)
          pbar.update(1)
          continue ##already wrong 
        if test['id_in_file'] not in perturbs[exp_label][str(budget)]:
          ...
          if example['attack_success']: attack_success += 1 
        else:
          # Required for progress bar to update correctly
          sleep(0.1)
        pbar.update(1)
  return attack_success/attack_attempted


def mnli_targeted_experiment(objective, model, inputs, file, min_budget, max_budget, maxiter, popsize, exp_label, overwrite,tokenizer,arch_type, max_seq_length):
  if overwrite or not exists(file):
    perturbs = { exp_label: { '0': dict() } }
  else:
    with open(file, 'rb') as f:
  ...
  for test in inputs:
    ...
    if test['id_in_file'] not in perturbs[exp_label]['0']:
      perturbs[exp_label]['0'][test['id_in_file']] = dict()
    for target in range(len(label_map)):
      if str(target) not in perturbs[exp_label]['0'][test['id_in_file']]:
        perturbs[exp_label]['0'][test['id_in_file']][str(target)] = {
              'adv_example': test['evidence'],
              'adv_example_enc': [],
              'input': test['evidence'],
              'claim': test['claim'],
  ...
  with tqdm(total=len(inputs)*(max_budget-min_budget+1)*len(label_map), desc="Adv. Examples") as pbar:
    for budget in range(min_budget, max_budget+1):
      if str(budget) not in perturbs[exp_label]:
        perturbs[exp_label][str(budget)] = dict()
      for input in inputs:
        if input['id_in_file'] not in perturbs[exp_label][str(budget)]:
          perturbs[exp_label][str(budget)][input['id_in_file']] = dict()
        for target in range(len(label_map)):
          if str(target) not in perturbs[exp_label][str(budget)][input['id_in_file']]:
            obj = objective(model, arch_type, tokenizer, input['evidence'], input['claim'], label_map[input['gold_label']], target, budget)
            example = obj.differential_evolution(verbose=False, maxiter=maxiter, popsize=popsize)
            perturbs[exp_label][str(budget)][input['id_in_file']][str(target)] = example
            with open(file, 'wb') as f:
              pickle.dump(perturbs, f)
          else:
            # Required for progress bar to update correctly
...
def load_mnli(model_type,model_checkpt):
  ...
  if model_type == 'bert':    
    ...
    model.load_state_dict(torch.load(model_checkpt)['model'])
    model = model.cuda()
    model.eval()
...
if __name__ == '__main__':
  ...
  parser.add_argument('--max_seq_length', type=int,default=150,help='maximum sequence length in words')
...
tokenizer, mnli = load_mnli(args.target_model_arch, args.target_model_chkpt)
...
if args.targeted:
    ...
    mnli_targeted_experiment(objective, mnli, data, args.pkl_file, args.min_perturbs, args.max_perturbs, args.maxiter, args.popsize, label, args.overwrite,tokenizer,args.target_model_arch, args.max_seq_length)
    ...
    success_ratio = mnli_experiment(mnli, objective, data, args.pkl_file, args.min_perturbs, args.max_perturbs, args.maxiter, args.popsize, label, args.overwrite,tokenizer,args.target_model_arch, args.max_seq_length)
