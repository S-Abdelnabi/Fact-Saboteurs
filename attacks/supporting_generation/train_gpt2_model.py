    ...
    with torch.no_grad():
        ...
        for batch in tqdm(dl, desc="Evaluation"):
            ...
            if input_ids.shape[0] != 4:
                break
            loss, logits, _ = model(input_ids, attention_mask=masks, labels=input_ids,return_dict=False)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train_pct", help="Percentage of data to use for training", type=float, default=1.0)
    ...
    parser.add_argument("--model_dir", help="Where to store the saved model", type=str)
    ...
    parser.add_argument("--target_class", help="The types of evidence to generate, either SUPPORTS or REFUTES, or both if --include_label", required=True, nargs='+')
    parser.add_argument("--warmup_steps", help="Number of steps to warm up Adam", type=int, default=200)
    parser.add_argument("--run_name", type=str, help="A name for the run", default="GPT2_evidence")
    ...
    dset = GPT2FeverDataset(args.target_class, args.dataset_loc, tokenizer, args.include_label)
    valdset = GPT2FeverDataset(args.target_class, args.val_dataset, tokenizer,args.include_label)
