...
from data_loader_replace_n import DataLoaderTest
    ...
    with open(attack_data_path) as fin:
        for step, line in enumerate(fin):
            ...
            for evidence in instance['evidence']:
                ...
                attack_success.append(int(evidence[3])) #evidence[3] is whether attack succeeded.
...
def check_against_top5(highest_5, evi_group):
    ...
    tot_success = np.sum(np.asarray(attack_success))  
...
def process_all_predict(all_predict_retrieved,evidence_to_check):    
    ...
    for key in evidence_to_check.keys():
        ...
        #print(tot_evidence_num)
        #print(tot_success_num)
        #print(count_in_top5)
        #print(count_success_in_top5)
...
parser.add_argument("--num_sent", type=int, default=2, help='max number of perturbed sentences to add')
...
validset_reader = DataLoaderTest(args.test_path_all_data, args.test_path_preattack, args.test_path_postattack, tokenizer, args, batch_size=args.batch_size, n=args.num_sent)
