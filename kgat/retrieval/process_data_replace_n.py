...
if __name__ == "__main__":
    ...
    preattack_sentences = read_file_groups_pre(args.retrieval_file_orig) ##original retrieval without anything
    postattack_sentences = read_file_groups_post(args.retrieval_file_orig_attacked) ##attack sentences . attack is conducted on the original retrieval 


    filter_dict = dict()
    data_dict = dict()
    golden_dict = dict()
    with open(args.gold_file) as f: #merge the items on the gold evidence to the retrieval 
    #check if the golden evidence was originally retrieved. 
    #if it was originally retrieved. take its counterpart from the attack sentences. 
    #if not found in the originally retrieved. add it. 
    #if in the test mode, golden evidence is not added, only the claim is added back. 
        for line in f:
        ...
        for line in f:
            ...
            for step, evidence in enumerate(data["evidence"]):
                ...
                if string not in golden_dict and string not in filter_dict:
                    data_dict[data["id"]]["evidence"].append([evidence[0], evidence[1], evidence[2], evidence[4], evidence[3]])
